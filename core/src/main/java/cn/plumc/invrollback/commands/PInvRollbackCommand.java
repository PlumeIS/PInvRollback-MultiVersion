...
public class PInvRollbackCommand implements TabExecutor {
    ...
    private boolean ui(Player player, String[] args) {
        if (args.length == 1) {
            RollbackUI.open(player, player.getUniqueId(), true);
            return true;
        } else if (args.length >= 2 || player.hasPermission("commands.pinvrollback.ui.other")) {
            OfflinePlayer target = Bukkit.getOfflinePlayer(args[1]);
            if (target.isOnline()) {
                RollbackUI.open(player, target.getUniqueId(), true);
            } else {
                RollbackUI.open(player, target.getUniqueId(), false);
            }
        } else {
            player.sendMessage(Config.i18n("command.permission_missing"));
            return true;
